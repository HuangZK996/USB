...
namespace USB
{
    ...
    {
        /// <summary>
        /// 遍历managementObject[name]或ManagementObject.GetPropertyValue(name)中name可输入的属性名
        /// </summary>
        /// <returns>设备属性名列表</returns>
        public static List<string> GetAllInfo()
        {
            ...
            foreach(ManagementObject Entity in UsbControllerDeviceCollection)
            {
        ...
        /// </summary>
        /// <returns></returns>
        public static List<string> GetAllUsbDevInfo()
        {
            PnPEntityInfo[] pnPEntityInfos = USBHelper.AllPnPEntities;
            if (pnPEntityInfos == null) return null;
            List<string> UsbdevList = new List<string>();
            foreach (PnPEntityInfo pnPEntityInfo in pnPEntityInfos)
            {
                ...
                UsbdevList.Add(string.Format("VID:PID  " + "{0:X4}:{1:X4}", pnPEntityInfo.VendorID, pnPEntityInfo.ProductID));
        ...
        ///// <summary>
        ///// 查询所有USB设备实体(不要求有VID和PID)
        ///// </summary>
        ///// <returns>USB实体列表</returns>
        //private static PnPEntityInfo[] WhoUsbDevice()
        //{
        ...
        //                    Element.VendorID = 0000;     // 供应商标识
        //                    Element.ProductID = 0000;   // 产品编号
        ...
        //        }
        //    }
        //        return (UsbDevices.Count > 0 ? null : UsbDevices.ToArray());
        //}
        /// <summary>
        ...
        private static PnPEntityInfo[] WhoUsbDevice(UInt16 VendorID, UInt16 ProductID, Guid ClassGuid)
        {
            List<PnPEntityInfo> UsbDevices = new List<PnPEntityInfo>();
            //获取USB控制器及其相关联的设备实体
            ManagementObjectCollection UsbControllerDeviceCollection = new ManagementObjectSearcher("SELECT * FROM Win32_USBControllerDevice").Get();
        ...
        public static PnPEntityInfo[] WhoPnPEntity(UInt16 VendorID, UInt16 ProductID, Guid ClassGuid)
        {
            ...
            if (ClassGuid == Guid.Empty)
                QueryString = "SELECT * FROM Win32_PnPEntity WHERE PNPDeviceID LIKE" +
                    VIDPID;
            else
                QueryString = "SELECT * FROM Win32_PnPEntity WHERE PNPDeviceID LIKE" +
                    VIDPID + " AND ClassGuid='" +ClassGuid.ToString("B") + "'";
        ...
        public static PnPEntityInfo[] WhoPnPEntity(String[] ServiceCollection)
        {
            ...
            if(PnPEntityCollection != null)
            {
                ...
                {
                    String PNPDeviceID = Entity["PNPDeviceID"] as String;
                    Match match = Regex.Match(PNPDeviceID, "VID_[0-9|A-F]{4}&[0-9|A-F]{4}");
                    if(match.Success)
                    {
                        String theService = Entity["Service"] as String;//服务
                        if (String.IsNullOrEmpty(theService)) continue;
